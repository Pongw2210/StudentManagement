...
from StudentManagement import app, db,max_student,max_age,min_age
...
class Student(Base):
    ...
    def validate_dob(self, key, dob_value):
        ...
        if age < min_age or age > max_age:
            raise ValueError(f"Tuổi học sinh phải từ {min_age} đến {max_age}.")
        return dob_value

class Class(Base):
    __tablename__="class"
    __table_args__ = (
        CheckConstraint(f'number_of_students <={max_student} ', name='check_max_students'),
        {'extend_existing': True}
    )
